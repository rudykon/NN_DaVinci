...
def inspect_pdf(
    ...
    temporary: Path,
    *,
    require_native_font_contract: bool = False,
) -> dict[str, Any]:
    ...
    normalized_font_names = {
        re.sub(r"^[A-Z]{6}\+", "", str(item["name"]))
        for item in fonts
    }
    base14_fonts = sorted(normalized_font_names.intersection(base14_names))
    all_fonts_embedded = bool(fonts) and all(item["embedded"] for item in fonts)
    all_fonts_unicode = bool(fonts) and all(item["unicode"] for item in fonts)
    if require_native_font_contract:
        if not all_fonts_embedded:
            failures.append("native Scene PDF contains an unembedded font")
        if not all_fonts_unicode:
            failures.append("native Scene PDF lacks a ToUnicode mapping")
        if base14_fonts:
            failures.append(f"native Scene PDF uses forbidden Base-14 fonts: {base14_fonts}")
    return {
        ...
        "all_fonts_unicode": all_fonts_unicode,
        "base14_fonts": base14_fonts,
        "native_font_contract_required": require_native_font_contract,
...
def compile_tikz(source: Path, destination: Path) -> tuple[Path | None, list[str]]:
    failures: list[str] = []
    # TeX may need to materialize bitmap fonts even for a completely offline
    # document.  Keep every generated cache/config file inside the oracle's
    # disposable case directory instead of depending on a writable $HOME.
    ...
    for variable, leaf in (
        ...
        directory = destination / leaf
        directory.mkdir(parents=True, exist_ok=True)
        tex_environment[variable] = str(directory)
    ...
    with tempfile.TemporaryDirectory(prefix="nndv-scene-publication-oracle-") as raw_temporary:
        ...
        for index, directory in enumerate(directories):
            ...
            pdf = inspect_pdf(
                ...
                labels,
                case_temporary / "pdf",
                require_native_font_contract=True,
            )
