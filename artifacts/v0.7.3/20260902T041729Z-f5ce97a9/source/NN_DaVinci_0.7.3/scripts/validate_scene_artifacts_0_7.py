#!/usr/bin/env python3
"""Independently validate the NN_DaVinci 0.7.0 Scene artifact corpus.
...
REPORT_SCHEMA = "nndv-0.7.0-scene-artifact-validation-1"
GENERATION_REPORT_SCHEMA = "nndv-0.7.0-scene-artifact-corpus-1"
...
def _inspect_png(path: Path, *, comparison: bool = False) -> tuple[dict[str, Any], list[str]]:
    ...
    if comparison and metadata.get("schema_version") != "nndv-0.7.0-2d-3d-comparison-1":
...
def _verify_generation_report(path: Path, results: list[dict[str, Any]]) -> tuple[dict[str, Any], list[str]]:
    ...
    if report.get("schema_version") != GENERATION_REPORT_SCHEMA or report.get("status") != "PASS":
        failures.append("generation report schema/status is not a passing 0.7.0 corpus report")
