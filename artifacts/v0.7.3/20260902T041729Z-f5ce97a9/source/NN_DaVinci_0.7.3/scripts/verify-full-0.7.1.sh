...
PARENT_SOURCE_DIR="${NNDV_PARENT_070_SOURCE:-$WORKSPACE_DIR/NN_DaVinci_0.7.0_Dev}"
PARENT_RUN_ID="20260831T044446Z-9e6cd3cf"
PARENT_SOURCE_FILES=356
PARENT_SOURCE_DIGEST="16aa60c611c4cf8c327fe4174314091dcb8dae840dc22ce85f73dae9926fb00d"
AUTHORITATIVE_070_ARTIFACT="${NNDV_AUTHORITATIVE_070_ARTIFACT:-$PARENT_SOURCE_DIR/artifacts/v0.7.0/$PARENT_RUN_ID}"
PARENT_SCENE_PERFORMANCE="$AUTHORITATIVE_070_ARTIFACT/reports/performance/scene-studio.json"
PARENT_SCENE_CORPUS="$AUTHORITATIVE_070_ARTIFACT/exports/scene-corpus"
ARTIFACT_ROOT="$PROJECT_DIR/artifacts/v0.7.1"
...
if [[ ! "$RUN_ID" =~ ^[A-Za-z0-9._-]+$ ]]; then
  echo "unsafe 0.7.1 run identifier: $RUN_ID" >&2
  exit 1
fi

CONTROL_DIR="$(mktemp -d "$TEMPORARY_ROOT/nndv-071-full-${RUN_ID}-XXXXXX")"
SNAPSHOT_DIR="$CONTROL_DIR/clean-source/NN_DaVinci_0.7.1"
PARENT_SNAPSHOT_DIR="$CONTROL_DIR/parent-source/NN_DaVinci_0.7.0"
STAGING_DIR="$CONTROL_DIR/artifact-staging"
WORK_DIR="$CONTROL_DIR/work"
SOURCE_REPORT="$STAGING_DIR/reports/source/snapshot.json"
PARENT_SOURCE_REPORT="$STAGING_DIR/reports/source/parent-0.7.0-snapshot.json"
PARENT_ARTIFACT_REPORT="$STAGING_DIR/reports/source/parent-0.7.0-artifact-verification.json"
...
report = {
    "schema_version": "nndv-0.7.1-full-command-report-1",
...
resolved = path.resolve(strict=True)
if resolved.parent != temporary or not resolved.name.startswith("nndv-071-full-"):
    raise SystemExit(f"refusing to clean unexpected full controller path: {resolved}")
shutil.rmtree(resolved)
PY
  else
    echo "0.7.1 full diagnostics retained: $CONTROL_DIR" >&2
  fi
  set -e
}
trap finish EXIT
...
if [[ ! -d "$PARENT_SOURCE_DIR" || -L "$PARENT_SOURCE_DIR" ]]; then
  echo "frozen 0.7.0 source root is absent or unsafe: $PARENT_SOURCE_DIR" >&2
  exit 1
fi
if [[ ! -d "$AUTHORITATIVE_070_ARTIFACT" || -L "$AUTHORITATIVE_070_ARTIFACT" ]]; then
  echo "authoritative 0.7.0 artifact is absent or unsafe: $AUTHORITATIVE_070_ARTIFACT" >&2
  exit 1
fi
if [[ "$(basename "$AUTHORITATIVE_070_ARTIFACT")" != "$PARENT_RUN_ID" ]]; then
  echo "authoritative parent artifact is not the pinned 0.7.0 run: $AUTHORITATIVE_070_ARTIFACT" >&2
  exit 1
fi
...
if [[ -L "$PROJECT_DIR/artifacts" \
  ...
  echo "0.7.1 artifact root is unsafe: $ARTIFACT_ROOT" >&2
  exit 1
fi
mkdir -p "$ARTIFACT_ROOT"
if [[ -L "$ARTIFACT_ROOT" || ! -d "$ARTIFACT_ROOT" ]]; then
  echo "0.7.1 artifact root could not be created safely: $ARTIFACT_ROOT" >&2
  exit 1
fi
...
if [[ -e "$ARTIFACT_ROOT/$RUN_ID" \
  ...
  echo "refusing to overwrite 0.7.1 run: $RUN_ID" >&2
  exit 1
fi
...
cd "$PROJECT_DIR"
run_logged clean-0.7.1-source-snapshot "$STAGING_DIR/logs/source-snapshot.log" \
...
run_logged independent-parent-0.7.0-source-snapshot "$STAGING_DIR/logs/parent-source-snapshot.log" \
...
run_logged parent-0.7.0-source-identity "$STAGING_DIR/logs/parent-source-identity.log" \
...
if not (
    ...
    raise SystemExit(
        "frozen 0.7.0 source identity mismatch: "
        + json.dumps(
            {
...
run_logged authoritative-parent-0.7.0-artifact-read-only "$STAGING_DIR/logs/parent-artifact-verification.log" \
  "$PYTHON_BIN" "$PARENT_SNAPSHOT_DIR/scripts/verify_release_artifact_0_7.py" \
  "$AUTHORITATIVE_070_ARTIFACT" --output "$PARENT_ARTIFACT_REPORT"

run_logged authoritative-parent-0.7.0-artifact-identity "$STAGING_DIR/logs/parent-artifact-identity.log" \
...
if not (
    ...
    raise SystemExit("authoritative 0.7.0 artifact identity or inventory verification failed")
print(f"parent artifact PASS run_id={report['run_id']} files={report['source_file_count']}")
PY

run_logged quick-0.7.1 "$STAGING_DIR/logs/quick.log" \
  env \
...
run_logged release-audit-0.7.1 "$STAGING_DIR/logs/release-audit.log" \
  "$PYTHON_BIN" scripts/audit_release_0_7_1.py \
...
run_logged fresh-fourteen-case-scene-generation "$STAGING_DIR/logs/scene-artifact-generation.log" \
  "$PYTHON_BIN" scripts/generate_scene_artifacts_0_7_1.py \
...
run_logged independent-scene-artifact-validation "$STAGING_DIR/logs/scene-artifact-validation.log" \
  "$PYTHON_BIN" scripts/validate_scene_artifacts_0_7_1.py \
  --output-dir "$STAGING_DIR/exports/scene-corpus" \
...
run_logged strict-fourteen-svg-chrome-oracle "$STAGING_DIR/logs/scene-svg-oracle.log" \
  node scripts/scene_svg_oracle_0_7_1.mjs \
...
run_logged independent-pdf-tikz-publication-oracle "$STAGING_DIR/logs/scene-publication-oracle.log" \
  "$PYTHON_BIN" scripts/scene_publication_oracle_0_7_1.py \
  --input-root "$STAGING_DIR/exports/scene-corpus" \
...
run_logged fourteen-case-0.7.0-to-0.7.1-before-after-proof "$STAGING_DIR/logs/scene-before-after.log" \
...
run_logged wheel-sdist-install-0.7.1 "$STAGING_DIR/logs/package-install.log" \
...
"$PYTHON_BIN" scripts/finalize_artifact_0_7_1.py \
  --staging "$STAGING_DIR" \
...
"$PYTHON_BIN" scripts/verify_release_artifact_0_7_1.py "$ARTIFACT_ROOT/$RUN_ID" \
...
print(
    ...
    f"inherited_0_7_0={python['inherited_0_7_0_tests']} "
