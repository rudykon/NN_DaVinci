...
if [[ -n "${NNDV_QUICK_RUN_DIR:-}" ]]; then
  ...
  RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nndv-060-quick-XXXXXX")"
  OWN_RUN=1
fi
STARTED="$(date +%s)"
...
finish() {
  ...
  if [[ "$OWN_RUN" -eq 1 && "$SUCCESS" -eq 1 && "$status" -eq 0 && "${NNDV_KEEP_DIAGNOSTICS:-0}" != "1" ]]; then
    ...
    echo "0.6 quick verification diagnostics: $RUN_DIR" >&2
  fi
}
trap finish EXIT
...
if [[ ! -x "$PROJECT_DIR/node_modules/.bin/eslint" ]]; then
  echo "node_modules is missing; run npm ci --ignore-scripts" >&2
  exit 1
fi
...
root = Path.cwd()
assert nn_davinci.__version__ == "0.6.0"
assert tomllib.loads((root / "pyproject.toml").read_text())["project"]["version"] == "0.6.0"
assert json.loads((root / "package.json").read_text())["version"] == "0.6.0"
lock = json.loads((root / "package-lock.json").read_text())
assert lock["version"] == lock["packages"][""]["version"] == "0.6.0"
for path in (root / "schemas").glob("*.json"):
    json.loads(path.read_text(encoding="utf-8"))
print("version/schema identity PASS")
PY
...
FIGURE_CORE_INCLUDE='src/nn_davinci/figure_ir.py,src/nn_davinci/tensor_geometry.py,src/nn_davinci/figure_export.py,src/nn_davinci/figure_templates.py,src/nn_davinci/structure_lens.py'
...
summary = {
    "schema_version": "nndv-0.6.0-quick-verification-1",
    ...
    "python": {
        ...
        "inherited_0_5_2_tests": 149,
        "new_0_6_0_tests": tests["collected"] - 149,
...
print("NNDV_060_QUICK_RESULT=" + json.dumps(summary, ensure_ascii=False, sort_keys=True))
PY
