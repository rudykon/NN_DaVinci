#!/usr/bin/env python3
"""Independent read-only verification for a sealed 0.7.0 artifact."""
...
from finalize_artifact_0_7 import REPORTS as REQUIRED_REPORTS
from finalize_artifact_0_7 import payload_failures, report_failures
...
def main() -> int:
    ...
    if not (
        manifest.get("schema_version") == "nndv-0.7.0-artifact-manifest-1"
    ...
    if not (
        verification.get("schema_version") == "nndv-0.7.0-verification-1"
    ...
    if not (
        ...
        failures.append("verification does not retain an exact all-pass 246-test parent baseline")
    ...
    result = {
        "schema_version": "nndv-0.7.0-artifact-inventory-verification-1",
    ...
    print("NNDV_070_ARTIFACT_VERIFY=" + json.dumps({
