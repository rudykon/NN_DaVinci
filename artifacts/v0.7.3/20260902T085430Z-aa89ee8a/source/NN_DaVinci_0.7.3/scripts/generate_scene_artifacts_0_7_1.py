#!/usr/bin/env python3
"""Generate the NN_DaVinci 0.7.1 editable Scene artifact corpus.
...
REPORT_SCHEMA = "nndv-0.7.1-scene-artifact-corpus-1"
...
def write_comparison_png(figure: FigureIR, scene_png: Path, destination: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="nndv-071-comparison-") as directory:
        ...
        with Image.open(figure_paths[0]) as figure_image, Image.open(scene_png) as scene_image:
            ...
            metadata.add_text(
                ...
                json.dumps(
                    {
                        "schema_version": "nndv-0.7.1-2d-3d-comparison-1",
                        "dpi": PNG_EXPORT_DPI,
