...
REPORT_SCHEMA = "nndv-0.7.0-scene-visual-evidence-validation-1"
SOURCE_SCHEMA = "nndv-0.7.0-scene-studio-e2e-1"
...
class SceneVisualEvidenceValidator:
    ...
    def _inspect_svg_routes(cls, elements: list[ET.Element]) -> tuple[JsonObject, list[str]]:
        failures: list[str] = []
        route_kinds = {"polyline", "route", "connector"}
        ...
        if not route_ids or not segments:
            failures.append("actual SVG contains no measurable data-kind route/polyline geometry")
        if intersections:
