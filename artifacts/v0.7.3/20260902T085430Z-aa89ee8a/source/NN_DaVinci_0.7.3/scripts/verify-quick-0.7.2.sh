...
if [[ -n "${NNDV_QUICK_RUN_DIR:-}" ]]; then
  ...
  RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nndv-072-quick-XXXXXX")"
  OWN_RUN=1
fi
STARTED="$(date +%s)"
...
finish() {
  ...
  if [[ "$OWN_RUN" -eq 1 && "$SUCCESS" -eq 1 && "$status" -eq 0 && "${NNDV_KEEP_DIAGNOSTICS:-0}" != "1" ]]; then
    ...
    echo "0.7.2 quick verification diagnostics: $RUN_DIR" >&2
  fi
}
trap finish EXIT
...
root = Path.cwd()
assert nn_davinci.__version__ == "0.7.2"
assert tomllib.loads((root / "pyproject.toml").read_text())['project']['version'] == "0.7.2"
assert json.loads((root / "package.json").read_text())['version'] == "0.7.2"
lock = json.loads((root / "package-lock.json").read_text())
assert lock['version'] == lock['packages']['']['version'] == "0.7.2"
...
baseline_path = root / "verification/fixtures/python-test-ids-0.7.1.txt"
baseline = baseline_path.read_text(encoding="utf-8").splitlines()
assert len(baseline) == len(set(baseline)) == 342
assert hashlib.sha256(baseline_path.read_bytes()).hexdigest() == "8d0c621a059f2d9a3fe4d0dadc2c127a7d56c236da98ae3048df131670676f49"
print("0.7.2 version / Project 1.4 / Scene IR 1.0 / 342-test parent baseline identity PASS")
PY
...
"$PYTHON_BIN" -m mypy \
  src/nn_davinci/architecture_evidence.py src/nn_davinci/architecture_parity.py \
  src/nn_davinci/scene_math.py src/nn_davinci/scene_ir.py \
  src/nn_davinci/model_scene.py src/nn_davinci/scene_projection.py \
  src/nn_davinci/scene_export.py src/nn_davinci/scene_gltf.py src/nn_davinci/pdf_fonts.py \
...
"$PYTHON_BIN" -m coverage run --branch --source=nn_davinci \
  ...
  --baseline-ids verification/fixtures/python-test-ids-0.7.1.txt
...
baseline_count = 342
parent_floors = {
    "old_core": 89.21365752715985,
    "expanded_core": 89.42561884162461,
    "scene_core": 82.08409506398537,
    "all_package": 84.48156482603427,
}
failures = []
if tests.get("baseline_count") != baseline_count or tests.get("baseline_ids_missing"):
    failures.append("one or more authoritative 0.7.1 Python test IDs are missing")
if not (
...
observed_coverage = {
    ...
    "scene_core": scene.get("percent_covered", 0.0),
    "all_package": coverage["all_package"]["combined"]["percent"],
}
for name, floor in parent_floors.items():
    if observed_coverage[name] + 1e-12 < floor:
        failures.append(
            f"{name} coverage regressed below authoritative 0.7.1: "
            f"{observed_coverage[name]:.12f}% < {floor:.12f}%"
        )
if failures:
    raise SystemExit("0.7.2 quick verification failed:\n- " + "\n- ".join(failures))
summary = {
    "schema_version": "nndv-0.7.2-quick-verification-1",
    ...
    "python": {
        ...
        "inherited_0_7_1_tests": baseline_count,
        "new_0_7_2_tests": tests["collected"] - baseline_count,
        "baseline_0_7_1_missing": tests["baseline_ids_missing"],
    ...
    "coverage": {
        ...
        "gates": {
            ...
            "authoritative_parent_floors_percent": parent_floors,
            "observed_percent": observed_coverage,
            "no_regression_from_0_7_1": True,
        },
    },
...
print("NNDV_072_QUICK_RESULT=" + json.dumps(summary, ensure_ascii=False, sort_keys=True))
PY
