#!/usr/bin/env python3
"""Audit 0.6.0 product identity, documentation, schemas, and template assets."""
...
def main() -> int:
    ...
    for surface, version in identities.items():
        if version != RELEASE:
            failures.append(f"{surface} identifies {version!r}, expected {RELEASE!r}")
    ...
    for relative in ("schemas/figure-ir-1.0.schema.json", "schemas/project-1.3.schema.json"):
        path = root / relative
        try:
            schemas[relative] = _load_json(path)
        except (OSError, ValueError, json.JSONDecodeError) as exc:
            failures.append(f"invalid required schema {relative}: {exc}")
    if not schemas.get("schemas/figure-ir-1.0.schema.json", {}).get("$id", "").endswith("figure-ir-1.0.schema.json"):
        failures.append("Figure IR schema does not carry its 1.0 identity")
    if not schemas.get("schemas/project-1.3.schema.json", {}).get("$id", "").endswith("project-1.3.schema.json"):
    ...
    for spec in TEMPLATE_SPECS:
        ...
        proof = figure.validate()
        templates[spec.slug] = {
            "public_sha256": _sha256(public),
            "packaged_sha256": _sha256(packaged),
            "panels": len(list(proof.iter_panels())),
            "modes": sorted({panel.mode for panel in proof.iter_panels()}),
        }
        if templates[spec.slug]["modes"] != ["schematic", "tensor-geometry"]:
            failures.append(f"template lacks both required modes: {spec.slug}")

    human = _load_json(root / "docs" / "trial" / "human-trial-status.json")
    ...
    if human_boundary != {
        ...
        "automation_counts_as_participants": False,
    }:
        failures.append("0.6.0 human-evidence boundary is not an honest zero-participant state")

    protocol = (root / "docs" / "trial" / "TRIAL_PROTOCOL.md").read_text(encoding="utf-8")
    protocol_compatibility = {
        "product_identity_0_6_0": "NN_DaVinci 0.6.0" in protocol,
        "protocol_1_0": "Protocol version 1.0" in protocol,
        "event_schema_0_5_0": "0.5.0-trial-events-1" in protocol,
        "compatibility_not_product_identity": "protocol compatibility, not the product build" in protocol,
        "old_evidence_not_migrated": "No 0.5.0, 0.5.1 or" in protocol,
    }
    if not all(protocol_compatibility.values()):
        failures.append("Trial protocol does not clearly separate compatible protocol IDs from the 0.6.0 product build")

    report = {
        "schema_version": "nndv-0.6.0-release-audit-1",
        ...
        "checks": {
            ...
            "trial_protocol_compatibility": protocol_compatibility,
        },
        "failures": failures,
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print("NNDV_060_RELEASE_AUDIT=" + json.dumps({"status": report["status"], "failures": len(failures)}, sort_keys=True))
