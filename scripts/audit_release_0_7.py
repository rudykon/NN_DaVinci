#!/usr/bin/env python3
"""Audit the user-visible 0.7.0 release contract without trusting artifacts."""
...
REQUIRED_DOCS = (
    ...
    "docs/FEATURE_COMPLETION_MATRIX_0.7.md",
)
...
def main() -> int:
    ...
    if set(versions.values()) != {"0.7.0", "1.4", "1.0"} or any(
        versions[key] != "0.7.0"
    ...
    for name in REQUIRED_DOCS:
        ...
        if "0.7.0" not in text and name != "docs/FIGURE_IR.md":
            stale_docs.append(name)
    if missing_docs:
        failures.append(f"required 0.7.0 docs are missing: {missing_docs}")
    if stale_docs:
        failures.append(f"required docs do not identify 0.7.0: {stale_docs}")
    ...
    if not all(checks["web"][key] for key in ("scene_canvas", "cpu_fallback", "command_palette", "project_schema_1_4")):
        failures.append("Web shell lacks one or more required Scene/command/schema surfaces")

    baseline = root / "verification" / "fixtures" / "python-test-ids-0.6.1.txt"
    ...
    if len(ids) != len(set(ids)) or len(ids) != 246 or baseline_sha != "62f9f1f1c1bf04ec44fac6688b4c7ac3e58127314b40e1e1e3fb790211be412d":
        failures.append("authoritative 0.6.1 test-ID fixture identity changed")
    ...
    print("NNDV_070_RELEASE_AUDIT=" + json.dumps({"status": report["status"], "failures": len(failures)}, sort_keys=True))
