#!/usr/bin/env python3
"""Finalize one passed, fresh 0.7.0 staging tree using existing sealing."""
...
REPORTS = {
    ...
    "parent_source": "reports/source/parent-0.6.1-snapshot.json",
    "parent_artifact": "reports/source/parent-0.6.1-artifact-verification.json",
...
def scene_e2e_failures(report: dict[str, Any]) -> list[str]:
    ...
    if not (
        report.get("schema_version") == "nndv-0.7.0-scene-studio-e2e-1"
...
def visual_evidence_failures(report: dict[str, Any], scene: dict[str, Any]) -> list[str]:
    ...
    if not (
        report.get("schema_version") == "nndv-0.7.0-scene-visual-evidence-validation-1"
        and report.get("release") == RELEASE
        and report.get("status") == "PASS"
...
def payload_failures(staging: Path, reports: dict[str, dict[str, Any]]) -> list[str]:
    ...
    if not (
        ...
        and source_claim.get("schema_version") == "nndv-0.7.0-scene-studio-e2e-1"
...
def report_failures(reports: dict[str, dict[str, Any]]) -> list[str]:
    ...
    if not (
        quick.get("schema_version") == "nndv-0.7.0-quick-verification-1"
        and quick.get("release") == RELEASE
        and passed(quick)
        and isinstance(python.get("collected"), int)
        and python.get("collected", 0) >= 246
        ...
        and python.get("inherited_0_6_1_tests") == 246
        and python.get("baseline_0_6_1_missing") == []
        and all(gates.get(name) is True for name in (
            "old_core_passed", "expanded_core_passed", "all_package_passed", "scene_core_80_percent",
        ))
    ):
        failures.append("quick verification is not an exact all-pass retaining 246 parent test IDs and coverage gates")
    ...
    if not (
        corpus.get("schema_version") == "nndv-0.7.0-scene-artifact-corpus-1"
    ...
    if not (
        validation.get("schema_version") == "nndv-0.7.0-scene-artifact-validation-1"
    ...
    if not (
        performance.get("schema_version") == "nndv-0.7.0-scene-studio-performance-1"
    ...
    audit = reports["audit"]
    if not (
        audit.get("schema_version") == "nndv-0.7.0-release-audit-1"
    ...
    if not (
        migration.get("schema_version") == "nndv-0.7.0-project-scene-migration-1"
    ...
    if not (
        ...
        and packaging.get("wheel_install", {}).get("version") == "0.7.0"
        and packaging.get("sdist_install", {}).get("version") == "0.7.0"
    ...
    if not (
        commands.get("schema_version") == "nndv-0.7.0-full-command-report-1"
    ...
    if not (
        ...
        and source.get("release") == "0.7.0"
        and isinstance(source.get("file_count"), int)
        and source.get("file_count", 0) > 294
        ...
        failures.append("fresh 0.7.0 clean-source snapshot is invalid")
    ...
    if not (
        environment.get("project_version") == "0.7.0"
...
def main() -> int:
    ...
    if not re.fullmatch(r"[A-Za-z0-9._-]+", args.run_id) or args.run_id in {".", ".."}:
        raise SystemExit(f"unsafe 0.7.0 run identifier: {args.run_id!r}")
    ...
    if failures:
        raise SystemExit("0.7.0 artifact preconditions failed:\n- " + "\n- ".join(failures))
    ...
    visual = reports["visual"]
    verification = {
        "schema_version": "nndv-0.7.0-verification-1",
        "release": RELEASE,
        "status": "PASS",
        "run_id": args.run_id,
    ...
    try:
        ...
        manifest = {
            "schema_version": "nndv-0.7.0-artifact-manifest-1",
            "release": RELEASE,
            "run_id": args.run_id,
        ...
        independent = subprocess.run(
            [
                sys.executable,
                str(args.project_root / "scripts" / "verify_release_artifact_0_7.py"),
                str(candidate),
                "--expected-run-id",
                args.run_id,
                "--output",
    ...
    print(f"NNDV_070_ARTIFACT={target}")
    return 0
