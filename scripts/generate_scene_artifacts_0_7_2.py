#!/usr/bin/env python3
"""Generate the NN_DaVinci 0.7.2 editable Scene artifact corpus.
...
VERSION = os.environ.get("NNDV_CORPUS_VERSION", "0.7.2")
RELEASE = os.environ.get("NNDV_CORPUS_RELEASE", "0.7.2 — Scientific Fidelity & Semantic Scene Hotfix")
REPORT_SCHEMA = os.environ.get("NNDV_CORPUS_SCHEMA", "nndv-0.7.2-scene-artifact-corpus-1")
...
def write_comparison_png(figure: FigureIR, scene_png: Path, destination: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="nndv-072-comparison-") as directory:
...
def generate_one(kind: str, key: str, destination: Path) -> dict[str, Any]:
    ...
    scientific_document_paths: list[Path] = []
    ...
    if kind == "real-model":
        ...
        graph_path = write_json(destination / "graph.graph.json", graph.to_dict())
        semantic_path = write_json(destination / "semantic.semantic.json", semantic.to_dict())
        figure_path = write_json(destination / "figure.figure.json", figure.to_dict())
        scene_path = next(path for path in export_paths if path.name == "scene.scene.json")
        landed_graph = GraphIR.from_dict(json.loads(graph_path.read_text(encoding="utf-8")))
        landed_semantic = SemanticView.from_dict(json.loads(semantic_path.read_text(encoding="utf-8")))
        landed_figure = FigureIR.from_dict(json.loads(figure_path.read_text(encoding="utf-8")))
        landed_scene = Scene.from_dict(json.loads(scene_path.read_text(encoding="utf-8")))
        landed_semantic.validate(landed_graph)
        parity_report = validate_landed_architecture_parity(
            landed_graph,
            landed_semantic,
            landed_figure,
            landed_scene,
        ...
        parity_report.update({
            ...
            "case_key": key,
            "landed_documents": {
                "graph": {"path": graph_path.name, "sha256": sha256(graph_path)},
                "semantic": {"path": semantic_path.name, "sha256": sha256(semantic_path)},
                "figure": {"path": figure_path.name, "sha256": sha256(figure_path)},
                "scene": {"path": scene_path.name, "sha256": sha256(scene_path)},
            },
        })
        parity_path = write_json(destination / "semantic-parity.json", parity_report)
        scientific_document_paths = [graph_path, semantic_path, figure_path]
        if not parity_report["passed"]:
            failures.append("independent landed Graph/Semantic/Figure/Scene architecture parity failed")
        comparison_paths = [
    ...
    evidence_paths = [*scientific_document_paths, *([parity_path] if parity_path is not None else [])]
    relative_artifacts = [
        path.relative_to(destination).as_posix()
        for path in [*export_paths, *comparison_paths, *evidence_paths]
    ]
    evidence_family = (
        semantic.architecture_evidence.family
    ...
    project = Project(
        ...
        model_source={
            "kind": kind,
            "key": key,
            "architecture": key if kind == "template" else evidence_family,
            "architecture_origin": "template" if kind == "template" else "architecture-evidence",
            "offline": True,
        ...
        artifacts=[
            {
                ...
                "role": (
                    ...
                    else "semantic-parity-report"
                    if path == "semantic-parity.json"
                    else "landed-scientific-document"
                    if path in {"graph.graph.json", "semantic.semantic.json", "figure.figure.json"}
                    else "scene-export"
                ),
            }
    ...
    all_paths = [*export_paths, *comparison_paths, *evidence_paths, project_path]
    ...
    return {
        ...
        "evidenced_architecture_family": evidence_family,
        "architecture_evidence": semantic.architecture_evidence.to_dict() if semantic is not None else None,
        "graph_digest": canonical_digest(graph.to_dict()),
        "semantic_digest": canonical_digest(semantic.to_dict()) if semantic is not None else None,
        ...
        "scientific_document_accounting": (
            {
                "graph": "graph.graph.json",
                "semantic": "semantic.semantic.json",
                "figure": "figure.figure.json",
                "scene": "scene.scene.json",
            }
            if kind == "real-model"
            else {}
        ),
        "semantic_parity_accounting": ["semantic-parity.json"] if parity_path is not None else [],
        "semantic_parity": parity_report,
        "omitted_critical_edges": {
            "figure": (
                figure.metadata.get("semantic_view", {})
...
def build_report(results: list[dict[str, Any]], output_dir: Path) -> dict[str, Any]:
    ...
    counts = {
        ...
        "landed_scientific_documents": sum(len(item.get("scientific_document_accounting", {})) for item in results),
        "semantic_parity_reports": sum(len(item.get("semantic_parity_accounting", [])) for item in results),
    ...
    expected_counts = {
        ...
        "landed_scientific_documents": 7 * 4,
