#!/usr/bin/env python3
"""Independently validate the NN_DaVinci 0.7.2 Scene artifact corpus.
...
from nn_davinci.architecture_parity import validate_landed_architecture_parity
from nn_davinci.figure_ir import FigureIR
from nn_davinci.ir import GraphIR
from nn_davinci.model_figure import validate_model_figure_provenance
...
from nn_davinci.semantic import SemanticView


REPORT_SCHEMA = os.environ.get("NNDV_VALIDATION_SCHEMA", "nndv-0.7.2-scene-artifact-validation-1")
GENERATION_REPORT_SCHEMA = os.environ.get("NNDV_CORPUS_SCHEMA", "nndv-0.7.2-scene-artifact-corpus-1")
COMPARISON_SCHEMA = os.environ.get("NNDV_COMPARISON_SCHEMA", "nndv-0.7.2-2d-3d-comparison-1")
...
SIGNATURES = {
    ...
    "graph.graph.json": (b"{", "landed-graph-json"),
    "semantic.semantic.json": (b"{", "landed-semantic-json"),
    "figure.figure.json": (b"{", "landed-figure-json"),
    "semantic-parity.json": (b"{", "semantic-parity-json"),
}
...
def _inspect_png(path: Path, *, comparison: bool = False) -> tuple[dict[str, Any], list[str]]:
    ...
    if comparison and metadata.get("schema_version") != COMPARISON_SCHEMA:
...
def validate_case(case_dir: Path, kind: str, key: str) -> dict[str, Any]:
    ...
    omitted_critical_edges: dict[str, Any] | None = None
    if kind == "real-model":
        try:
            landed_graph = GraphIR.from_dict(
                json.loads((case_dir / "graph.graph.json").read_text(encoding="utf-8"))
            )
            landed_semantic = SemanticView.from_dict(
                json.loads((case_dir / "semantic.semantic.json").read_text(encoding="utf-8"))
            )
            landed_figure = FigureIR.from_dict(
                json.loads((case_dir / "figure.figure.json").read_text(encoding="utf-8"))
            )
            landed_semantic.validate(landed_graph)
            project_semantic = project.persisted_semantic_view()
            landed_documents_match_project = bool(
                project_semantic is not None
                and landed_graph.to_dict() == project.graph.to_dict()
                and landed_semantic.to_dict() == project_semantic.to_dict()
                and landed_figure.to_dict() == project.figure_ir
                and scene.to_dict() == project.scene_ir
            )
            if not landed_documents_match_project:
                failures.append("landed Graph/Semantic/Figure/Scene documents disagree with Project 1.4")
            figure_provenance = validate_model_figure_provenance(
                landed_figure,
                landed_graph,
                landed_semantic,
            )
            scene_provenance = validate_model_scene_provenance(
                scene,
                landed_graph,
                landed_semantic,
                landed_figure,
            ...
            provenance = {
                "schema_version": "nndv-four-document-provenance-validation-1",
                "figure": figure_provenance,
                "scene": scene_provenance,
                "passed": bool(figure_provenance.get("passed") and scene_provenance.get("passed")),
            }
            if not provenance["passed"]:
                failures.append("landed Figure/Scene provenance is invalid")
            recomputed_parity = validate_landed_architecture_parity(
                landed_graph,
                landed_semantic,
                landed_figure,
                scene,
            ...
            stored_parity = json.loads(
                (case_dir / "semantic-parity.json").read_text(encoding="utf-8")
            )
            mismatched_parity_fields = sorted(
                name
                for name, value in recomputed_parity.items()
                if stored_parity.get(name) != value
            )
            expected_landed_documents = {
                "graph": {"path": "graph.graph.json", "sha256": sha256(case_dir / "graph.graph.json")},
                "semantic": {"path": "semantic.semantic.json", "sha256": sha256(case_dir / "semantic.semantic.json")},
                "figure": {"path": "figure.figure.json", "sha256": sha256(case_dir / "figure.figure.json")},
                "scene": {"path": "scene.scene.json", "sha256": sha256(case_dir / "scene.scene.json")},
            }
            parity_validation = {
                **recomputed_parity,
                "stored_report_matches_recomputation": not mismatched_parity_fields,
                "stored_report_mismatched_fields": mismatched_parity_fields,
                "stored_landed_documents_match": stored_parity.get("landed_documents") == expected_landed_documents,
            }
            parity_validation["passed"] = bool(
                recomputed_parity.get("passed")
                and not mismatched_parity_fields
                and stored_parity.get("landed_documents") == expected_landed_documents
                and stored_parity.get("case_kind") == kind
                and stored_parity.get("case_key") == key
            )
            if not parity_validation["passed"]:
                failures.append("independent landed scientific-semantic parity validation failed")
            evidence = landed_semantic.architecture_evidence
            architecture_family = evidence.family if evidence is not None else None
            if architecture_family in {None, "unknown"}:
                failures.append("real model lacks non-unknown source-bound Architecture Evidence")
            omitted_critical_edges = {
                "figure": (
                    landed_figure.metadata.get("semantic_view", {})
                    .get("publication_projection", {})
                    .get("omitted_critical_edges")
                ),
                "scene": scene.metadata.get("model_scene_pipeline", {}).get("omitted_critical_edges"),
            }
            if omitted_critical_edges != {"figure": 0, "scene": 0}:
                failures.append("Figure or Scene omitted one or more critical architecture edges")
        except Exception as exc:
            provenance = {"passed": False, "error": f"{type(exc).__name__}: {exc}"}
            parity_validation = {"passed": False, "error": f"{type(exc).__name__}: {exc}"}
            failures.append(f"four-document scientific validation raised {type(exc).__name__}: {exc}")
    else:
    ...
    return {
        ...
        "evidenced_architecture_family": architecture_family,
        "semantic_parity_validation": parity_validation,
        "omitted_critical_edges": omitted_critical_edges,
...
def _verify_generation_report(path: Path, results: list[dict[str, Any]]) -> tuple[dict[str, Any], list[str]]:
    ...
    if report.get("schema_version") != GENERATION_REPORT_SCHEMA or report.get("status") != "PASS":
        failures.append("generation report schema/status is not a passing current corpus report")
...
def validate_corpus(output_dir: Path, generation_report: Path | None = None) -> dict[str, Any]:
    ...
    counts = {
        ...
        "landed_scientific_documents": sum(
            sum(
                name in item.get("files", {})
                for name in (
                    "graph.graph.json",
                    "semantic.semantic.json",
                    "figure.figure.json",
            ...
            for item in results
            if item.get("kind") == "real-model"
        ),
        "semantic_parity_reports": sum(
            "semantic-parity.json" in item.get("files", {})
            and bool((item.get("semantic_parity_validation") or {}).get("passed"))
            for item in results
        ),
        "non_unknown_real_models": sum(
            item.get("kind") == "real-model"
            and item.get("evidenced_architecture_family") not in {None, "unknown"}
            for item in results
        ),
        "zero_critical_omission_real_models": sum(
            item.get("kind") == "real-model"
            and item.get("omitted_critical_edges") == {"figure": 0, "scene": 0}
            for item in results
        ),
        "regular_files": sum(len(item.get("files", {})) for item in results),
    }
    expected_counts = {
        "cases": 14,
        "templates": 7,
