...
PARENT_SOURCE_DIR="${NNDV_PARENT_071_SOURCE:-$WORKSPACE_DIR/NN_DaVinci_0.7.1_Dev}"
...
AUTHORITATIVE_071_ARTIFACT="${NNDV_AUTHORITATIVE_071_ARTIFACT:-$PARENT_SOURCE_DIR/artifacts/v0.7.1/$PARENT_RUN_ID}"
PARENT_SCENE_PERFORMANCE="$AUTHORITATIVE_071_ARTIFACT/reports/performance/scene-studio.json"
PARENT_SCENE_CORPUS="$AUTHORITATIVE_071_ARTIFACT/exports/scene-corpus"
ARTIFACT_ROOT="$PROJECT_DIR/artifacts/v0.7.2"
...
if [[ ! "$RUN_ID" =~ ^[A-Za-z0-9._-]+$ ]]; then
  echo "unsafe 0.7.2 run identifier: $RUN_ID" >&2
  exit 1
fi

CONTROL_DIR="$(mktemp -d "$TEMPORARY_ROOT/nndv-072-full-${RUN_ID}-XXXXXX")"
SNAPSHOT_DIR="$CONTROL_DIR/clean-source/NN_DaVinci_0.7.2"
PARENT_SNAPSHOT_DIR="$CONTROL_DIR/parent-source/NN_DaVinci_0.7.1"
STAGING_DIR="$CONTROL_DIR/artifact-staging"
WORK_DIR="$CONTROL_DIR/work"
SOURCE_REPORT="$STAGING_DIR/reports/source/snapshot.json"
PARENT_SOURCE_REPORT="$STAGING_DIR/reports/source/parent-0.7.1-snapshot.json"
PARENT_ARTIFACT_REPORT="$STAGING_DIR/reports/source/parent-0.7.1-artifact-verification.json"
...
report = {
    "schema_version": "nndv-0.7.2-full-command-report-1",
...
resolved = path.resolve(strict=True)
    if resolved.parent != temporary or not resolved.name.startswith("nndv-072-full-"):
    raise SystemExit(f"refusing to clean unexpected full controller path: {resolved}")
shutil.rmtree(resolved)
PY
  else
    echo "0.7.2 full diagnostics retained: $CONTROL_DIR" >&2
  fi
  set -e
}
trap finish EXIT
...
if [[ ! -d "$PARENT_SOURCE_DIR" || -L "$PARENT_SOURCE_DIR" ]]; then
  echo "frozen 0.7.1 source root is absent or unsafe: $PARENT_SOURCE_DIR" >&2
  exit 1
fi
if [[ ! -d "$AUTHORITATIVE_071_ARTIFACT" || -L "$AUTHORITATIVE_071_ARTIFACT" ]]; then
  echo "authoritative 0.7.1 artifact is absent or unsafe: $AUTHORITATIVE_071_ARTIFACT" >&2
  exit 1
fi
if [[ "$(basename "$AUTHORITATIVE_071_ARTIFACT")" != "$PARENT_RUN_ID" ]]; then
  echo "authoritative parent artifact is not the pinned 0.7.1 run: $AUTHORITATIVE_071_ARTIFACT" >&2
  exit 1
fi
if [[ ! -f "$PARENT_SCENE_PERFORMANCE" || -L "$PARENT_SCENE_PERFORMANCE" ]]; then
  echo "authoritative parent Scene performance report is absent or unsafe" >&2
  exit 1
fi
if [[ -L "$PROJECT_DIR/artifacts" \
  ...
  echo "0.7.2 artifact root is unsafe: $ARTIFACT_ROOT" >&2
  exit 1
fi
mkdir -p "$ARTIFACT_ROOT"
if [[ -L "$ARTIFACT_ROOT" || ! -d "$ARTIFACT_ROOT" ]]; then
  echo "0.7.2 artifact root could not be created safely: $ARTIFACT_ROOT" >&2
  exit 1
fi
...
if [[ -e "$ARTIFACT_ROOT/$RUN_ID" \
  ...
  echo "refusing to overwrite 0.7.2 run: $RUN_ID" >&2
  exit 1
fi
...
mkdir -p \
  ...
  "$STAGING_DIR/evidence/publication-before-after" \
  "$STAGING_DIR/exports" \
...
cd "$PROJECT_DIR"
run_logged clean-0.7.2-source-snapshot "$STAGING_DIR/logs/source-snapshot.log" \
  "$PYTHON_BIN" scripts/create_clean_snapshot.py "$PROJECT_DIR" "$SNAPSHOT_DIR" \
  --allowlist verification/source-allowlist-0.7.2.json \
  --reject-symlinks \
  --output "$SOURCE_REPORT"

run_logged independent-parent-0.7.1-source-snapshot "$STAGING_DIR/logs/parent-source-snapshot.log" \
  "$PYTHON_BIN" "$PARENT_SOURCE_DIR/scripts/create_clean_snapshot.py" \
  "$PARENT_SOURCE_DIR" "$PARENT_SNAPSHOT_DIR" \
  --allowlist verification/source-allowlist-0.7.1.json \
  --reject-symlinks \
  --output "$PARENT_SOURCE_REPORT"

run_logged parent-0.7.1-source-identity "$STAGING_DIR/logs/parent-source-identity.log" \
...
if not (
    ...
    raise SystemExit(
        "frozen 0.7.1 source identity mismatch: "
        + json.dumps(
            {
...
run_logged authoritative-parent-0.7.1-artifact-read-only "$STAGING_DIR/logs/parent-artifact-verification.log" \
  "$PYTHON_BIN" "$PARENT_SNAPSHOT_DIR/scripts/verify_release_artifact_0_7_1.py" \
  "$AUTHORITATIVE_071_ARTIFACT" --output "$PARENT_ARTIFACT_REPORT"

run_logged authoritative-parent-0.7.1-artifact-identity "$STAGING_DIR/logs/parent-artifact-identity.log" \
...
if not (
    ...
    raise SystemExit("authoritative 0.7.1 artifact identity or inventory verification failed")
print(f"parent artifact PASS run_id={report['run_id']} files={report['source_file_count']}")
PY

run_logged quick-0.7.2 "$STAGING_DIR/logs/quick.log" \
  env \
  NN_DAVINCI_PYTHON="$PYTHON_BIN" \
  NNDV_QUICK_RUN_DIR="$WORK_DIR/quick" \
  NNDV_KEEP_DIAGNOSTICS=1 \
  ./scripts/verify-quick-0.7.2.sh
...
run_logged release-audit-0.7.2 "$STAGING_DIR/logs/release-audit.log" \
  "$PYTHON_BIN" scripts/audit_release_0_7_2.py \
...
run_logged project-1.3-to-1.4-migration "$STAGING_DIR/logs/project-migration.log" \
  "$PYTHON_BIN" scripts/verify_scene_migration_0_7_1.py \
  --project-root "$SNAPSHOT_DIR" \
...
run_logged scene-studio-performance-three-repeats "$STAGING_DIR/logs/scene-performance.log" \
  "$PYTHON_BIN" scripts/benchmark_scene_studio_0_7_1.py \
  --output "$STAGING_DIR/reports/performance/scene-studio.json" \
  --baseline-scene-report "$PARENT_SCENE_PERFORMANCE" \
  --parent-source "$PARENT_SNAPSHOT_DIR" \
  --repeats 3

run_logged fresh-fourteen-case-scene-generation "$STAGING_DIR/logs/scene-artifact-generation.log" \
  "$PYTHON_BIN" scripts/generate_scene_artifacts_0_7_2.py \
...
run_logged independent-scene-artifact-validation "$STAGING_DIR/logs/scene-artifact-validation.log" \
  "$PYTHON_BIN" scripts/validate_scene_artifacts_0_7_2.py \
  --output-dir "$STAGING_DIR/exports/scene-corpus" \
...
run_logged strict-fourteen-svg-chrome-oracle "$STAGING_DIR/logs/scene-svg-oracle.log" \
  node scripts/scene_svg_oracle_0_7_2.mjs \
  --input-root "$STAGING_DIR/exports/scene-corpus" \
  --output "$STAGING_DIR/reports/visual/scene-svg-oracle.json"

run_logged independent-pdf-tikz-publication-oracle "$STAGING_DIR/logs/scene-publication-oracle.log" \
  "$PYTHON_BIN" scripts/scene_publication_oracle_0_7_2.py \
  --input-root "$STAGING_DIR/exports/scene-corpus" \
  --svg-report "$STAGING_DIR/reports/visual/scene-svg-oracle.json" \
  --output "$STAGING_DIR/reports/visual/scene-publication-oracle.json" \
  --expected-cases 14

run_logged fourteen-case-0.7.1-to-0.7.2-before-after-proof "$STAGING_DIR/logs/scene-before-after.log" \
  "$PYTHON_BIN" scripts/generate_scene_before_after_0_7_1.py \
  --before-root "$PARENT_SCENE_CORPUS" \
  --after-root "$STAGING_DIR/exports/scene-corpus" \
  --output-root "$STAGING_DIR/evidence/publication-before-after" \
  --report "$STAGING_DIR/reports/visual/scene-before-after.json" \
  --before-release 0.7.1 \
  --after-release 0.7.2 \
  --before-run-id "$PARENT_RUN_ID" \
  --release-title "0.7.2 — Scientific Fidelity & Semantic Scene Hotfix" \
  --report-schema nndv-0.7.2-scene-before-after-report-1 \
  --proof-schema nndv-0.7.2-scene-before-after-proof-1 \
  --allow-identical

run_logged visual-oracle-positive-negative-mutations "$STAGING_DIR/logs/visual-oracle-fixtures.log" \
  node scripts/validate_visual_oracle_fixtures_0_7_1.mjs \
  --output "$STAGING_DIR/reports/visual/oracle-fixtures.json"
...
run_logged independent-scene-visual-validation "$STAGING_DIR/logs/scene-visual-validation.log" \
  "$PYTHON_BIN" scripts/validate_scene_visual_evidence_0_7_1.py \
...
run_logged wheel-sdist-install-0.7.2 "$STAGING_DIR/logs/package-install.log" \
...
"$PYTHON_BIN" scripts/finalize_artifact_0_7_2.py \
  --staging "$STAGING_DIR" \
...
"$PYTHON_BIN" scripts/verify_release_artifact_0_7_2.py "$ARTIFACT_ROOT/$RUN_ID" \
...
print(
    ...
    f"inherited_0_7_1={python['inherited_0_7_1_tests']} "
