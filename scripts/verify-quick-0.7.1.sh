...
if [[ -n "${NNDV_QUICK_RUN_DIR:-}" ]]; then
  ...
  RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nndv-071-quick-XXXXXX")"
  OWN_RUN=1
fi
STARTED="$(date +%s)"
...
finish() {
  ...
  if [[ "$OWN_RUN" -eq 1 && "$SUCCESS" -eq 1 && "$status" -eq 0 && "${NNDV_KEEP_DIAGNOSTICS:-0}" != "1" ]]; then
    ...
    echo "0.7.1 quick verification diagnostics: $RUN_DIR" >&2
  fi
}
trap finish EXIT
...
root = Path.cwd()
assert nn_davinci.__version__ == "0.7.1"
assert tomllib.loads((root / "pyproject.toml").read_text())['project']['version'] == "0.7.1"
assert json.loads((root / "package.json").read_text())['version'] == "0.7.1"
lock = json.loads((root / "package-lock.json").read_text())
assert lock['version'] == lock['packages']['']['version'] == "0.7.1"
...
baseline_path = root / "verification/fixtures/python-test-ids-0.7.0.txt"
baseline = baseline_path.read_text(encoding="utf-8").splitlines()
assert len(baseline) == len(set(baseline)) == 328
assert hashlib.sha256(baseline_path.read_bytes()).hexdigest() == "a66646fea24b51f1d39432adac615de612186791300c1bec4296731b0ee83dac"
print("0.7.1 version / Project 1.4 / Scene IR 1.0 / 328-test parent baseline identity PASS")
PY
...
"$PYTHON_BIN" -m coverage run --branch --source=nn_davinci \
  ...
  --baseline-ids verification/fixtures/python-test-ids-0.7.0.txt
...
if tests.get("baseline_count") != baseline_count or tests.get("baseline_ids_missing"):
    failures.append("one or more authoritative 0.7.0 Python test IDs are missing")
if not (
...
if failures:
    raise SystemExit("0.7.1 quick verification failed:\n- " + "\n- ".join(failures))
summary = {
    "schema_version": "nndv-0.7.1-quick-verification-1",
    ...
    "python": {
        ...
        "inherited_0_7_0_tests": baseline_count,
        "new_0_7_1_tests": tests["collected"] - baseline_count,
        "baseline_0_7_0_missing": tests["baseline_ids_missing"],
...
print("NNDV_071_QUICK_RESULT=" + json.dumps(summary, ensure_ascii=False, sort_keys=True))
PY
